...
// [0] = 串级距离环(PosCascade用): 距离误差(cm)→速度指令(cm/s)。
//   ★占位值, 待 docs/pos_design.m 用实车辨识模型算出后回抄★
//   语义: v_cmd = Kp*剩余距离 + Ki*∫剩余距离。Kp≈8→误差5.6cm时给满速45cm/s(P主导天然减速零超调)。
p_i_d_Value_TypeDef PID_Value_Position_position[4] = {{ 8.0, 0.5, 0.0 },  // [0] 距离环(cm→cm/s) 占位, MATLAB替换
													  { 0.48, 0.0, 0.1 },  // [1] 未用
													  { 0.48, 0.0, 0.1 },  // [2] 未用
													  { 0.48, 0.0, 0.1 }};  // [3] 未用
...
p_i_d_Value_TypeDef PID_Value_Speed[4] = {
    { 2.8156, 0.4253, 4.4087 },  // [0] 左前轮 L1 (MATLAB辨识)
    { 2.6231, 0.4310, 3.7798 },  // [1] 右前轮 R1 (MATLAB辨识)
    { 1.18888, 0.2555, 0.1 },  // [2] 左后轮 (未用)
    { 1.18888, 0.2555, 0.1 }}; // [3] 右后轮 (未用)
