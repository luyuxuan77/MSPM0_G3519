...
int setup_imu(int use_ln, int accel_en, int gyro_en)
{
    ...
    // 添加 WHO_AM_I 读取重试
    for (whoami_retry = 0; whoami_retry < 3; whoami_retry++) {
        rc = lsm6dsv16x_device_id_get(&lsm6dsv_ctx, &whoami);
        if (rc == 0 && whoami == LSM6DSV16X_ID) {
            break; // 成功
        }
        // 失败则等待再试
        delay_ms(10);
        SPI1_CS_IMU(1); // 确保 CS 释放
    }

    if (whoami_retry >= 3 || rc != 0 || whoami != LSM6DSV16X_ID) {
        printf("LSM6DSV WHO_AM_I=0x%02x, expect 0x%02x after retries\r\n",
