...
void motor_control_update(void)
{
    ...
    // 内环现在32ms一拍(原128ms)。×4归一化成"128ms当量速度", 使 SetPoint/增益/限幅/里程
    // 全部无需改动, 对外仍是128ms语义, 只是电机响应快4倍。
    speed_M1 = (int16_t)(cnt[0]-last_cnt_M1) * 4;  // 左轮 (×4: 32ms差分→128ms当量)
    speed_M2 = -(int16_t)(cnt[1]-last_cnt_M2) * 4;  // 右轮 (负号: 编码器方向相反)
